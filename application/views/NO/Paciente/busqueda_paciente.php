                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <form class="form" id="frmBusquedaPaciente" action="<?php echo base_url(); ?>Paciente/resultado_busqueda" method="post">
                                <input type="text" class="form-control" placeholder="Nombre del paciente" id="busquedaPaciente" name="busquedaPaciente" required>
                            </form>
                        </div>
                    </div>
				</div>
...
<script>
    ...
    /* $(document).on("keydown", "#busquedaPaciente", function(event) {
            ...
            success:function(respuesta){
                var registro = eval(respuesta);
                    if (registro.length > 0){
						var html = '';
						html += '<table id="tabla-pacientes" class="table table-striped thead-primary w-100 tablaPlus">';
						html += '	<thead>';
						html += '		<th class="text-center">Nombre</th>';
						html += '		<th class="text-center">Edad</th>';
						html += '		<th class="text-center">Teléfono</th>';
						html += '		<th class="text-center">DUI</th>';
						html += '		<th class="text-center">Dirección</th>';
						html += '		<th class="text-center">Opción</th>';
						html += '	</thead>';
						html += '	<tbody>';
                        for (let i = 0; i < registro.length; i++) {
							//console.log(registro[i]["nombrePaciente"]);
							html += '<tr>';
							html += '	<td class="text-center">'+registro[i]["nombrePaciente"]+'</td>';
							html += '	<td class="text-center">'+registro[i]["edadPaciente"]+'</td>';
							html += '	<td class="text-center">'+registro[i]["telefonoPaciente"]+'</td>';
							html += '	<td class="text-center">'+registro[i]["duiPaciente"]+'</td>';
							html += '	<td class="text-center">'+registro[i]["direccionPaciente"]+'</td>';
							html += '	<td class="text-center">';
							html += '		<a title="Ver detalles del médico" href="#"><i class="fas fa-eye ms-text-primary"></i></a>';
							html += '		<a title="Actualizar datos del médico" href="#actualizarP"><i class="fas fa-edit ms-text-success"></i></a>';
							html += '		<a><i class="far fa-trash-alt ms-text-danger"></i></a>';
							html += '	</td>';
							html += '</tr>';
						}
						html += '	</tbody>';
						...
						$("#contenedorPaciente").html('');
						$("#contenedorPaciente").html(html);
						// console.log(html);
                    }
                }
        });
